...
const DataCity3D = lazy(() =>
  import("../components/dashboard/DataCity3D").then((module) => ({
    default: module.DataCity3D,
  })),
);
...
function VisualFallback({
  label = "در حال آماده‌سازی نمایش...",
}: {
  label?: string;
}) {
  return (
    <div className="visual-fallback">
      <span className="visual-fallback-dot" />
      <small>{label}</small>
    </div>
  );
}
      ...
      <div className="hero-panel">
        <div className="hero-copy">
          <div className="eyebrow">
            <i /> 3D SALES INTELLIGENCE
          </div>
          <h2>
            از عددها عبور کن؛
            <br />
            <span>فروش را در سه بُعد ببین.</span>
          </h2>
          <p>
            داشبورد مدیریتی جامع برای تحلیل فروش، سود، سفارش‌ها، مشتریان و
            کانال‌های درآمدی و مشاهده سریع عملکرد کسب‌وکار.
          </p>{" "}
        ...
        <div className="hero-visual">
          <Suspense
            fallback={
              <VisualFallback label="در حال بارگذاری نمای سه‌بعدی..." />
            }
          >
            <DataCity3D values={trend} />
          </Suspense>
          <div className="floating-metric metric-a">
            <small>رشد فروش</small>
            <b>+{s.revenueGrowth}٪</b>
          </div>
          <div className="floating-metric metric-b">
            <small>سفارش‌ها</small>
            <b>{formatNumber(s.totalOrders)}</b>
          </div>
      ...
      <div className="analytics-grid">
        <article className="panel wide">
          <div className="panel-head">
            <div>
              <small>روند عملکرد</small>
              <h3>فروش و سود در طول زمان</h3>
            </div>
            <span className="tag">میلیون تومان</span>
          </div>
          <div className="chart-lg">
            <Suspense
              fallback={<VisualFallback label="در حال آماده‌سازی نمودار..." />}
            >
        ...
        <article className="panel">
          <div className="panel-head">
            <div>
              <small>ترکیب درآمد</small>
              <h3>فروش بر اساس دسته</h3>
            </div>
          </div>
          <div className="chart-donut">
            ...
            <div className="donut-label">
              <small>بیشترین سهم</small>
              <b>{data.categorySales[0].value}٪</b>
            </div>
          </div>
          <div className="legend-list">
            {data.categorySales.slice(0, 4).map((x, i) => (
              <div key={x.name}>
                <i className={`c${i}`} />
                <span>{x.name}</span>
                <b>{x.value}٪</b>
        ...
        <article className="panel">
          <div className="panel-head">
            <div>
              <small>کانال فروش</small>
              <h3>درآمد هر کانال</h3>
            </div>
          </div>
          <div className="chart-sm">
            <Suspense
              fallback={<VisualFallback label="در حال آماده‌سازی نمودار..." />}
            >
              <ChannelChart data={data.channelSales} />
            </Suspense>
          </div>
        </article>
        <article className="panel">
          ...
          <div className="region-list">
            {data.regions.map((r) => (
              <div className="region" key={r.name}>
                <div>
                  <span>{r.name}</span>
                  <b>{formatMoney(r.revenue)}</b>
                </div>
                <div className="track">
                  <i style={{ width: `${r.share}%` }} />
                </div>
                <small>{formatNumber(r.orders)} سفارش</small>
              </div>
            ))}
          </div>
        </article>
          ...
          <div className="product-list">
            {products?.slice(0, 5).map((p, i) => (
              <div className="product-row" key={p.id}>
                ...
                <span className={`product-cube cube-${i}`} />
                <div>
                  <b>{p.name}</b>
                  <small>{p.category}</small>
                </div>
                <div className="end">
                  <b>{formatMoney(p.revenue)}</b>
                  <small className={p.growth >= 0 ? "positive" : "negative"}>
                    {p.growth > 0 ? "+" : ""}
                    {p.growth}٪
                  </small>
                </div>
